...
!function ($) {
    ...
    var createFormCommon = function (pColumns, that) {
        ...
        for (var i in pColumns) {
            ...
            if (!vObjCol.checkbox && vObjCol.field !== 'operate' && vObjCol.searchable && vObjCol.operator !== false) {
               // console.log(vObjCol.operator);
                var query = "";
                query = query ? query : '';
                ...
                htmlForm.push(sprintf('<input type="hidden" autocomplete="off" class="form-control operate" name="field-%s" data-name="%s" value="%s" readonly>', vObjCol.field, vObjCol.field, vObjCol.operate));
                ...
                if (vObjCol.searchList) {
                    ...
                    if (/BETWEEN$/.test(vObjCol.operate)) {
                        ...
                        htmlForm.push(sprintf('<div class="col-xs-6 11"><input autocomplete="off" type="%s" class="%s" name="%s" value="%s" placeholder="%s" id="%s" data-index="%s" %s %s></div>', type, addclass, vObjCol.field, defaultValueArr[0], placeholderArr[0], vObjCol.field, i, style, data));
                        htmlForm.push(sprintf('<div class="col-xs-6 22"><input autocomplete="off" type="%s" class="%s" name="%s" value="%s" placeholder="%s" id="%s" data-index="%s" %s %s></div>', type, addclass, vObjCol.field, defaultValueArr[1], placeholderArr[1], vObjCol.field, i, style, data));
                        htmlForm.push('</div>');
                    } else {
                        htmlForm.push(sprintf('<input type="%s"  autocomplete="off" class="%s" name="%s" value="%s" placeholder="%s" id="%s" data-index="%s" %s %s>', type, addclass, vObjCol.field, defaultValue, placeholder, vObjCol.field, i, style, data));
                    }
                }
